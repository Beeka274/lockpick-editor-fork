...
func _ready():
	is_ready = true
	for color_name in Enums.COLOR_NAMES.keys():
		if color_name == Enums.colors.none: continue
		if color_name == Enums.colors.gate and !support_gates: continue
...
func set_to_color(to_color: Enums.colors) -> void:
	for l in locks:
		if l.lock_data.color == to_color:
			selected_lock = l
			return
